...
TEST_F(TestSetup, TestPrivmsgEdgeCases)
{
    ...
    EXPECT_TRUE(outputContains("PRIVMSG user1 :Hello myself"));
    ...
    EXPECT_TRUE(outputContains("PRIVMSG #test :Hello, world! @#$%^&*()"));
    ...
    sendCommand(client1, "PRIVMSG user2,#test :Hello everyone");
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    EXPECT_TRUE(outputContains(":user1!testuser@127.0.0.1 PRIVMSG user2 :Hello everyone"));
    EXPECT_TRUE(outputContains(":user1!testuser@127.0.0.1 PRIVMSG #test :Hello everyone"));

    // Sending a message to a channel with a key
    sendCommand(client1, "JOIN #keyedChannel");
    sendCommand(client1, "MODE #keyedChannel +k key123");
    sendCommand(client2, "JOIN #keyedChannel key123");
    sendCommand(client2, "PRIVMSG #keyedChannel :Hello with key");
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    EXPECT_TRUE(outputContains(":user1!testuser@127.0.0.1 PRIVMSG #keyedChannel :Hello with key"));
